...
\begin{document}
    ...
    (z1.port4) to[short, -o] ++(-0.5,0) coordinate (Z1A)
    (z1.port3) to[short, -o] ++(0.5,0) coordinate (Z1B)
    (z1.port2) to[short, -o] ++(0.5,0) coordinate (Z1C)
    (z1.port1) to[short, -o] ++(-0.5,0) coordinate (Z1D)
    (Z1A) to [open, v^>=Port 1] (Z1D)
    (Z1B) to [open, v^>=Port 2] (Z1C)
    ;
    \node[align=center] at (z1) {Noisy \\ ABCD};
    \end{tikzpicture}
\end{document}