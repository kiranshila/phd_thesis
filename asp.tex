...
For this, we take the RF power and use it to directly modulate the current of a solid-state diode laser, effectively intensity modulating the light.
After traveling through up to \qty{20}{\km} of single mode fiber, this signal is presented to a photodiode, where it is converted back into RF current.
This style of analog RF transmission is used in cable television and commercial/military antenna remoting applications.
It is important to note here that direct modulation is similar, but architecturally very different from digital modulation approaches, where 1s and 0s are achieved by pulsing the laser, nearing 100\% modulation depth.
...
The noise at the input is taken as thermal noise in the generator impedance at a standard reference temperature of $T_0$\qty{=290}{\K}.
...
In the simple, direct-modulated diode link we are studying, RIN and shot noise are the dominant terms, and we will ignore the various sources of thermal noise~\cite{coxLimitsPerformanceRFoverfiber2006}.

The relative intensity noise of the laser (RIN) is the mean deviation in optical output power about the mean square output.
\begin{equation}
...
Unfortunately, this spectrum is not quite white, unlike pure shot noise.
Near the laser's relaxation resonance, RIN
is amplified~\cite{huiChapter1Fundamentals2023}, giving a slope to the power spectral density.
Other device-dependent effects also can strongly shape the spectral and bias-dependent effects.
As the behavior of RIN is strongly laser-dependent, it makes the most sense to start with an experiment measuring our laser's RIN.

To measure the laser RIN, we employ the ``subtraction method''~\cite{kashevskyMethodsMeasuringRelative2023}.
This method sidesteps the need for an optical spectrum analyzer by measuring the induced photocurrent noise due to the intensity noise on an RF spectrum analyzer.
...
Finally, as this noise power is proportional to $\overline{p_L^2}$, we divide by DC photocurrent's equivalent power to compute RIN.
\begin{equation}
...
We can repeat this measurement as a function of bias.
...
To validate some assumptions about RIN, we look at the bias (optical power) response at a few frequencies.
To see the dependence on laser power instead of laser bias current, however, we need to transform the laser current to power via the laser's threshold current $I_{th}$ and slope efficiency $s$.
...
Looking at the response in \autoref{fig:laser_pi}, we see that these datasheet values and our assumptions agree.
...
In \autoref{fig:rin_v_power} shows the RIN versus laser power at a few different frequencies.
We see the steep increase in RIN at low laser power due to spontaneous emission, and the linear section at high power due to shot noise at higher power, matching our prediction.

To aid in analysis, we can ignore the frequency dependence for now and fit a simple quadratic model at \qty{1.5}{\GHz}
\begin{equation}
...
Now that we understand the bias and frequency response of RIN, we can continue with our noise analysis.
...
We can simplify this circuit by combining the dependent sources and moving the noise currents forward towards the output, Port 2.
The total current gain between the laser input and the photodiode is
\begin{equation}
    g_i = sG_O\eta
\end{equation}
Noise in the optical power of the laser will certainly manifest as noise in the photocurrent, scaled by the square of the passive gain of the fiber and the photodiode responsivity, $(G_O\eta)^2$.
As the shot noise and RIN are uncorrelated, the total noise current is simply their sum.
\begin{equation}
...
However, the subtle distinction here is that moving forward is asking, "how does current changing at the input branch manifest as current at the output", which depends only on the gain, whereas moving backwards is asking, "what equivalent current at the input would manifest as current at the output", which depends on the circuitry attached to the input.

We can begin to solve this problem by working backwards, starting with the current we want at the input under the source termination condition and working towards the output.
...
\end{equation}
Then, we can use the current gain to move to the output
\begin{equation}
    i_\text{Out} = g_ii_\text{In}\frac{R_S}{R_L+R_S}
\end{equation}
Finally, we can solve for $i_\text{In}$ in terms of $i_\text{Out}$, and take the time average of the square
\begin{equation}
    \overline{i_\text{In}^2} = \frac{\overline{i_\text{Out}^2}}{g_i^2}\left(\frac{R_L}{R_S}+1\right)^2
\end{equation}

To find the noise figure, we can use the relation
\begin{equation}
...
Combining these last few equations yields
\begin{equation}
    F = 1 + \frac{\overline{i_\text{Out}^2}\left(\frac{R_L}{R_S}+1\right)^2}{4k_BT_0G_SBg_i^2}
\end{equation}
Finally, as we like to work in equivalent noise temperature, we use $T_e = T_0(F-1)$, we find
\begin{equation}
    T_e  = \frac{PR_S}{4k_Bs^2G_O\eta}\left(PG_O\eta\cdot\text{RIN} + 2q\right)\left(\frac{R_L}{R_S}+1\right)^2
    \label{eq:link_noise}
\end{equation}

This equivalent noise temperature result gives interesting insight into the behavior of the link under different source impedances.
If we are free to choose $R_S$ through matching, we would want to know what $R_S$ provides the best noise performance.
We can solve for this quantity by minimizing \autoref{eq:link_noise}, finding the $R_S$ that solves $\delta T_e/\delta R_s=0$.
...
The interesting conclusion from this plot is that for laser powers between \qty{8}{\mW} and \qty{12}{\mW}, the noise is about the same, within a factor of 2, and remains that way as we increase the loss.
...
In analyzing the gain and reflections of the RFoF link, it is a bit more convenient to analyze S parameters instead of voltages and currents.
...
$S_{21}$, however, is a bit more confusing.
If we plug some numbers into this equation, if $Z_0$=\qty{50}{\ohm}, $R_L$=\qty{5}{\ohm} and $s\eta G_\text{Opt}$=\num{0.2}, $|S_{21}|^2$=\qty{-8.79}{dB}.
If we ``match'' the laser by setting the characteristic impedance equal to the laser's impedance $R_L=Z_0$, this equation reduces to just $s\eta G_\text{Opt}$.
However, using the same parameters, we find $|0.2|^2$=\qty{-13.98}{\dB}, unintuitively lower gain than before.
...
For a real load as we are exploring, we can use an ideal transformer at the input, which has an ABCD matrix of
\begin{equation}
    A_{TF} = \begin{bmatrix}
        N & 0\\0 & 1/N
    \end{bmatrix}
\end{equation}
We can convert the Y matrix of the link to ABCD parameters using the standard equation to arrive at
\begin{equation}
    A_\text{Link} = -\frac{1}{s\eta G_\text{O}}\begin{bmatrix}
        0 & R_L \\ 0 & 1
    \end{bmatrix}
\end{equation}
And then cascade to get
\begin{equation}
    A_\text{Combined} = -\frac{1}{s\eta G_\text{O}}\begin{bmatrix}
        0 & NR_L \\ 0 & 1/N
    \end{bmatrix}
\end{equation}
And then finally converting to S parameters
\begin{equation}
...
One of the interesting consequences of this result is we can end up with positive power gain in the right conditions, even though the intrinsic link has current gain less than unity.
For example, if $s\eta G_O$\num{=0.2}, $Z_0$\qty{=50}{\ohm}, and $R_L$\qty{=2}{\ohm}, the link's gain $|S_{21}|$ would be unity.
While unintuitive, the transformer acts as a current gain device which for a given turns ratio will counteract the current loss in the link.

From both the noise and gain perspective, we want to power match the input of the laser.

\subsection{Linearity}

The final metric we evaluate for the RFoF link is its linearity.
...
Of course, one way to reduce the impact of this distortion is to limit the incoming signal strength.
Doing so, however, is at odds with added noise, as we want to \textit{increase} power before a noisy component to reduce that component's noise's impact on the SNR.
To compare RFoF links (bias settings, laser or photodiode choice, etc.), we can use ``spurious free dynamic range'' or (SFDR) as a noise-normalized linearity metric.
...
In addition to this ``hot'' measurement, we also measured the noise of the receiver, including the ``dark current'' of the photodiode in the same manner as the RIN measurement.
Then, we subtracted the linear noise powers to leave us with just the noise of the link.
Using measured S parameters from a network analyzer, we referred the noise power back to the input.

For this experiment, we used an AGx \textit{PLMR3-A-1-5-S} laser.
We measured the slope resistance (or equivalent input impedance) by measuring $S_{11}$ on the network analyzer, shown in \autoref{fig:laser_s11}.
%
\begin{figure}[ht!]
    \centering
    \includestandalone[mode=buildnew]{figures/laser_s11}
    \caption{$S_{11}$ of connectorized, biased laser, and input impedance at \qty{1.5}{\GHz}}
    \label{fig:laser_s11}
\end{figure}
%
This result is slightly confusing, as the laser is attached to a section of an SMA connector, which is itself an impedance transformer.
We measured an open circuit on a connector of the same length to de-embed the effect of the connector.
We measured an electrical length of \qty{44}{\degree} at \qty{1.5}{\GHz}.
...
The input-referred noise temperature in \autoref{fig:link_noise_in} matches our expected result from \autoref{fig:link_noise}, with reasonably high laser currents resulting in an input noise temperature of about \qty{60000}{\K} in the band of interest.
...
While it is simple to measure noise and gain to validate analytic expressions, linearity is more difficult as we do not have an expression for the actual transfer function on the laser and photodiode pair.
...
For the experiment, we inject two closely spaced tones, $\omega_L$ and $\omega_H$ (\qty{1}{\MHz} here) and observe five tones on the output.
We observe the powers of the two fundamental tones to track the gain, $P_L$ and $P_H$, $\omega_L+\omega_H$ for $\text{IM}_2$ to compute $\text{OIP}_2$ via
\begin{equation}
    \text{OIP2}~(\text{dBm}) = P_L+P_H-\text{IM2}
\end{equation}
and $2\omega_H-\omega_L$ for $\text{IM}_{3H}$, and $2\omega_L-\omega_H$ for $\text{IM}_{3L}$ to compute $\text{OIP}_{3}$ via
\begin{align}
...
In the presented range, the gain only slightly changes, from an average of \qty{-5}{\dB} to \qty{-4}{\dB} across the band, and as such the input values change accordingly.
The linearity improves through increased laser power, but then worsens as the device starts to saturate, near \qty{50}{\mA}.

Combining these noise and linearity measurements, we compute the spurious-free dynamic range as defined in \autoref{eq:sfdr}, shown in \autoref{fig:rfof_sfdr}.
...
Here, we see that we are limited by the 2nd-order linearity primarily, although it is important to note that after \qty{1}{\GHz}, the power in the second order tone (\qty{2}{\GHz}) is out of band and will be filtered out).
At high laser powers, in the operation band of \SIrange{0.7}{2}{\GHz}, we can expect SFDR of better than \qty{60}{\dB} for the intrinsic RFoF link.

As the SFDR falls off at high laser power while the noise improves, we can find the optimum laser bias, shown in \autoref{fig:sfdr2_sweep}.
...
As the lasers operate at \qty{1310}{\nm}, the effective loss in standard single-mode fiber is around \qty{0.5}{\dB/\km}.
...
We show that adding the isolators is an effective way to reduce the nonlinearities introduced by Rayleigh backscattering by repeating the earlier tests on the bench with a \qty{20}{\km} spool\footnote{Thank you, Courtney!}, shown in \autoref{fig:scattering_linearity}.
In this figure, the top row are tests at \qty{0}{\km} of fiber, with the laser and photodiode directly connected.
The bottom row includes the \qty{20}{\km} spool of single-mode fiber.
...
From \autoref{fig:scattering_linearity}, it is clear that at \qty{20}{\km} we experience Rayleigh scattering as the linearity for the single-isolated laser is significantly lower.
Adding the double isolator appears to improve the linearity for both second and third order distortion.
...
We can see the same story in noise, measuring the devices in the same manner as described earlier.
...
With the single-isolator laser, the noise increases by more than an order of magnitude greater when we add the \qty{20}{\km} of fiber, which is much more than the predicted results from \autoref{fig:link_noise}.
...
Additionally, it does not seem to be a significant detriment to performance in the short-fiber distances.
...
First, and arguably most important, is the survey speed.
...
As the LNA has \qty{\approx35}{\dB} of gain, this implies a maximum of \qty[group-digits = false]{\approx3100}{\K} at the output of the LNA.

Next, we can focus on the gain.
The amount of absolute power required to drive the digitizer is of course dependent on the exact digitizer we use.
...
We want to set the gain of the analog signal path such that the astronomical signal exercises three bits~\cite{bianchi2008adc}.
As the ADC has quantization noise, a detectable signal must be above that noise floor, giving rise to the ``effective number of bits'' (ENOB) metric.
This ADC has an ENOB of about 9 at \qty{2}{\GHz}.
...
The signal levels from the astronomical sources will be incredibly faint, so their intermodulation products are unimportant.
...
The total gain is \qty{74.7}{\dB}, with adjustable attenuators allowing for control, setting the adjustable attenuators to match our gain requirement to maximize dynamic range.
...
\begin{figure}[ht!]%
    \centering
    \subfloat{\includegraphics[height=4cm]{figures/ftx.jpg}}%
    \quad%
    \subfloat{\includegraphics[height=4cm]{figures/frx.jpg}}%
    \caption{FTX and FRX PCBs}%
    \label{fig:ftx_frx}%
\end{figure}

To implement this analog signal path, we designed two printed circuit boards, shown in \autoref{fig:ftx_frx}.
The fiber transmitter (FTX) board at the antenna station and the fiber receiver (FRX) board at the central processing facility.
...
In the current design stage, it is unclear if we will require much filtering outside the general band shape.
In earlier stages of the project, strong interference causing significant nonlinearity was a top concern.
The current RFI environment of the DSA-2000 site seems to indicate that no other filtering will be required.
...
This larger card hosts the ADCs and FPGA and fits into a 10-unit Eurocard subrack.
...
The design of this card was straightforward, with a standard assortment of filters and amplifiers.
Perhaps the most interesting component is the distributed low pass antialiasing filter at the output.
This filter is an inverse-Chebyshov design that has been optimized in physical area.
...
The presented LNA from \autoref{ch:lna} is very close to the theoretical minimum noise of the transistor, with typical losses of the matching network.
...
An alternative option for hybrid-amplifiers is to cool just the first-stage transistor, as that has the highest impact on the LNA's noise.
...
Modern advancements in miniaturization of Peltier coolers for optoelectronics\footnote{Typically used for lasers in high-speed digital communication} have dramatically lowered cost and improved performance of these devices.
As such, we can design an amplifier with an embedded microcooler for the first stage transistor to improve the noise without significant impact to cost or power consumption.
...
The potential noise gains, however, need to be weighted against the complications due to creating a below-freezing point inside the amplifier.
...
To prevent this, we need to design an LNA that is hermetically sealed, which adds significant complexity and cost.
...
Models for this transistor~\cite{DiramicsPH2004F50} at \qty{2}{\GHz} indicate a minimum noise temperature of \qty{7}{\K} at \qty{300}{\K} ambient and \qty{0.6}{\K} at \qty{15}{\K} ambient.
This implies a noise slope of about \qty{1}{\K} per \qty{45}{\K} in ambient temperature.
To improve our amplifier by \qty{1}{\K}, we can integrate a small single-stage microcooler to generate a \qty{45}{\K} differential.

To fully estimate the heat load, we need to account for heat introduced from the short wire bonds.
...
This implies that the bulk of our heat requirement comes from the wire bonds and not the transistor.
...
Estimations from the manufacturer for their \texttt{1TC22-0044-25.H} model predicts a maximum $\Delta T$ of just beneath \qty{30}{\K} for \qty{45}{\mW} of heat.
...
A close-up picture of the bonded transistor and thermistor on the microcooler and assembled amplifier is shown in \autoref{fig:lna_cooling}.
The transistor's wire bonds from left to right are the gate, source, and drain.
The two wire bonds on the bottom connect to the thermistor.
...
While this experiment was a success, eventually ice did start to form on the first stage, destroying the fragile air bridges.
Work on this active-cooling approach needs further investigation in environmental control, especially as it pertains to mass-manufacture for the 4000 LNAs that would eventually need to be built.
Additionally, our original estimations of the heat load which influenced the sizing of the microcoolers did not include the heat load of the wire bonds.
...
This would increase wire bond length which would reduce the heat load, but add significant inductance which will, of course, affect our matching network.
...
To reduce the effect of the feed impedance ``pulling'' the noise, we can design an amplifier that is optimized directly to the feed impedance.
If this target impedance were a large deviation from \qty{50}{\ohm}, it would be difficult to validate performance in the lab, as all the noise experiments are done in a \qty{50}{\ohm} system.
...
Performing this optimization follows the work from \autoref{ch:lna}, with a small modification made to the definition of the noise goal in the cost expression.
...
\end{equation}
where $\Gamma_S$ is that measured $S_{11}$ (assuming everything is normalized to the same reference impedance).
...
As the feed operates in a slotline mode, there is a pin balun that transitions from coax to slotline.
...
While this separation of components is useful from a maintenance perspective, the long coax pin that attaches the connector to the throat of the feed introduces additional loss.
...
A lid gasket and hermetic output connector allows for solid-state cooling.
...
As discussed in \autoref{ch:lna}, this is at odds with the insertion loss of the line itself, implying an optimum compromise between the length of the line providing a quality noise match and the loss of the line.
When co-optimizing with the DSA-2000 feed, this optimum has shifted to favor longer lines.
In short, the design can tolerate the loss introduced from the longer line, as the noise introduced due to mismatch at the band edges is so great.
