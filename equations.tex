...
\end{equation}
For system noise temperature $T_\text{Sys}$, bandwidth $\Delta\nu$, and integration time $\tau$, we compute the RMS error (in Kelvin).
If we want to know the signal-to-noise ratio for an observation of a point source, we define the ratio of the source's induced antenna temperature $T_a$ over this RMS noise level as
\begin{equation}
...
We then multiply this by $N$ antennas to compute the total effective collecting area for the entire array.
\begin{equation}
    A_e = \frac{N\eta_a\pi D^2}{4}\,.
\end{equation}
Now, we find the integration time required to achieve an SNR of unity as
\begin{equation}
...
In other words, we find what flux density ($S_\nu$) we can measure at a $1\sigma_T$-level in some fixed integration time $\tau$ via
\begin{equation}
...
\end{equation}
Substituting into \autoref{eq:intermod_expansion}, we find
\begin{align}
...
\end{align}
Here, we see that increasing powers from the series expansion leads to sum and difference frequencies of the form $n\omega_1 \pm m\omega_2$ for positive integers $m$ and $n$.
...
\end{equation}
The definition of the intercept point is when the powers at the fundamental and the intermodulation product are equal, given by
\begin{equation}
...
Given two components with power gains $G_1$ and $G_2$ with output nth-order output intercept points $\text{OIP}n^\prime$ and $\text{OIP}n^{\prime\prime}$, we want to find the effect of combining their distortions.
...
In that case, we treat the contributions as incoherent and directly sum the powers of the intermodulation products.
\begin{equation}
...
\end{equation}
Again solving for the total cascaded 2nd order intercept, incoherently combined gives
\begin{equation}
    \text{OIP2} = \left(
    \frac{1}{G_2\text{OIP2}^{\prime}} + \frac{1}{\text{OIP2}^{\prime \prime}}
    \right)^{-1}\,.
